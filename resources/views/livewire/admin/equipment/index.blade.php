    ...
    <div class="card" style="margin: 2% 2%">
                ...
                <tbody>
                    @forelse ($items as $index => $item)
                        <tr>
                            ...
                            <td class="text-danger">{{ $item->broken_quantity }}</td>

                            <td class="text-success fw-bold">{{ $item->actual_quantity }}</td>
                            ...
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('admin.equipment.edit', ['id' => $item->equipment->id, 'lab_id' => $item->lab_id]) }}"
                                            class="dropdown-item">
                                            <i class="ph-note-pencil px-1"></i> Chỉnh sửa
                                        </a>

                                        <a href="#" wire:click.prevent="openDeleteModal({{ $item->id }})"
        ...
        <div class="card-footer bg-white d-sm-flex justify-content-sm-between align-items-sm-center py-3">
            <div class="text-muted">
                @if($items->total() > 0)
                    Hiển thị từ {{ $items->firstItem() }} đến {{ $items->lastItem() }} trong tổng số {{ $items->total() }}
                    kết quả
                @else
                    Không có dữ liệu
                @endif
