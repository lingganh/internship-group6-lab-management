<div>
                        ...
                        <div class="diary-filters mb-3">
                            <div class="row g-2 g-md-3 align-items-end">
                                ...
                                <div class="col-12 col-md-3">
                                    ...
                                    <input type="text" wire:model.live="keyword" class="form-control diary-control"
                                        ...
                                        <tr wire:key="event-{{ $item->id }}">
                                            <td>
                                                <div class="fw-semibold text-dark text-truncate"
                                                    style="max-width: 520px;">{{ $item->title }}</div>
                                                <div class="small text-muted">#{{ $item->id }} •
                                                    {{ $this->categoryLabel($item->category) }}</div>
                                            </td>

                                            <td>
                                                <div class="fw-semibold text-dark">
                                                    {{ $item->lab?->name ?? ($item->lab_code ?? 'N/A') }}</div>
                                                <div class="small text-muted">{{ $item->lab_code ?? '-' }}</div>
                                            </td>

                                            <td>
                                                <div class="fw-semibold text-dark">
                                                    {{ $item->user?->full_name ?? 'N/A' }}</div>
                                                <div class="small text-muted">{{ $item->user?->email ?? '' }}</div>
                                            </td>

                                            <td>
                                                <div class="fw-semibold text-dark">{{ $item->start->format('d/m/Y') }}
                                                </div>
                                                <div class="small text-muted">{{ $item->start->format('H:i') }} –
                                                    {{ $item->end->format('H:i') }}</div>
                                            </td>
                                            ...
                                            <td class="text-end">
                                                <button wire:click="viewEvent({{ $item->id }})"
                                                    class="btn btn-sm diary-btn diary-btn-primary" type="button">
                                                    Chi tiết
                                                </button>
                                            </td>
                                        </tr>
                  ...
                  <div class="diary-filebox">
                    ...
                    @endif
                                ...
                                <div class="col-12">
                                    <div class="diary-filebox">
                                        <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
                                            <div class="fw-bold text-dark">Phiếu báo hỏng</div>
                                            <div class="small text-muted">
                                                {{ $selectedEvent->issueRequests?->count() ?? 0 }} phiếu
                                            </div>
                                        </div>

                                        @if ($selectedEvent->issueRequests && $selectedEvent->issueRequests->count())
                                            <div class="diary-filelist">
                                                @foreach ($selectedEvent->issueRequests as $req)
                                                    @php
                                                        ...
                                                        $stClass = match ($st) {
                                                            'pending' => 'diary-pill diary-pill-pending',
                                                            'in_review' => 'diary-pill diary-pill-pending',
                                                            'completed' => 'diary-pill diary-pill-approved',
                                                            'cancelled' => 'diary-pill diary-pill-cancelled',
                                                            default => 'badge bg-secondary',
                                                        };
                                                    @endphp

                                                    <a class="diary-fileitem"
                                                        href="{{ route('admin.equipment-issue-requests.show', $req->id) }}"
                                                        ...
                                                        <span class="w-100">
                                                            <div
                                                                ...
                                                                <div class="fw-bold">
                                                                    Phiếu #{{ $req->id }}
                                                                    • {{ (int) ($req->items_count ?? 0) }} thiết bị
                                                                    •
                                                                    {{ optional($req->created_at)->format('d/m/Y H:i') }}
                                                                </div>

                                                                <span
                                                                    class="{{ $stClass }}">{{ $stLabel }}</span>
                                                            </div>
                                        ...
                                        @else
                                            <div class="small text-muted">Chưa có phiếu báo hỏng cho lần sử dụng này.
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="modal-footer border-0 pt-0">
                        <div class="d-flex w-100 justify-content-between align-items-center gap-2">
                            <button type="button" class="btn diary-btn diary-btn-ghost"
                    ...
                    <div class="modal-footer border-0 pt-0">
                        <div class="d-flex w-100 justify-content-end gap-2">
                            ...
                            <button wire:click="deleteEvent" type="button"
                                class="btn diary-btn diary-btn-danger">Xác nhận xóa</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <style>
        ...
        .diary-page .diary-btn-ghost:hover {
            background: #f8fafc;
        }
